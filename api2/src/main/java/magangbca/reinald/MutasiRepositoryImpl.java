...
public class MutasiRepositoryImpl implements MutasiRepository {
    ...
    @Override
        ...
        resp.setResult(storedProcedureResults.stream().map(result -> new Mutasi( result[0].toString(),result[2].toString(),result[1].toString(),result[3].toString(),result[4].toString(),(BigInteger) result[5])).collect(Collectors.toList()));
        return resp;
    }
}
